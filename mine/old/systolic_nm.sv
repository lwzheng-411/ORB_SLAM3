// Top
module systolicarray_back_moreIP #(
    parameter N = 6,
    parameter M = 18
)(
    input   wire clk,
    ...
    localparam integer TOTAL_QR_CYCLES   = M + 3 * N - 2; // generalized: when M==N -> 4*N-2
    ...
    assign qr_enable_src = (state == S_QR_DECOMP) && (cycle_count < M);
            ...
            case (state)
                ...
                S_QR_DECOMP: begin
                    if (cycle_count >= TOTAL_QR_CYCLES) begin
                        state <= S_DONE; // skip back-substitution for QR-only
                        cycle_count <= '0;
                    end else begin
    ...
    assign enable_src = (state == S_QR_DECOMP) && (inject_row < M);
      ...
      for (col = 0; col <= N; col = col + 1) begin : input_col
        if (col == 1) begin : NON_DATA_COL
            ...
            if (num_buf == 0) begin : NO_DELAY
                assign y_flow[0][col] = (state == S_QR_DECOMP && inject_row < M && matrix_col < N) ? matrix_in[inject_row][matrix_col] :
                                       (state == S_QR_DECOMP && inject_row >= M && matrix_col < N) ? matrix_in[M-1][matrix_col] : 32'h0;
            end else begin : BUFFER
                wire [31:0] buffer_chain [0:num_buf];
                assign buffer_chain[0] = (state == S_QR_DECOMP && inject_row < M && matrix_col < N) ? matrix_in[inject_row][matrix_col] :
                                        (state == S_QR_DECOMP && inject_row >= M && matrix_col < N) ? matrix_in[M-1][matrix_col] : 32'h0;
        ...
        for (b_row = 0; b_row < N; b_row = b_row + 1) begin : b_vertical_flow
            if (b_row == 0) begin : FIRST_ROW_B_INJECT
                ...
                if (b_delay == 0) begin : NO_B_DELAY
                    ...
                    assign b_flow[b_row] = (state == S_QR_DECOMP && inject_row < M) ? b_in[inject_row] : 32'h0;
                end else begin : B_BUFFER
                    wire [31:0] b_buffer_chain [0:b_delay];
                    // Drive with b_in[inject_row] during the M-cycle injection window
                    assign b_buffer_chain[0] = (state == S_QR_DECOMP && inject_row < M) ? b_in[inject_row] : 32'h0;
...
module divider (
    ...
    // BackSub: xi = back_sum_in / r (dedicated divider)
    DW_fp_div #(
        .sig_width(23),
        .exp_width(8),
        .ieee_compliance(1)
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            xi_reg <= (r_out_bottom == 32'h00000000) ? 32'h0 : xi_temp;
        end
    end
...
module rotation (
    ...
    // Dedicated multipliers/subtractors for QR and BackSub (no reuse)
    wire [31:0] sr1_qr, sr1_back;
    wire [31:0] r2_prime_qr, r2_prime_back;
    ...
    // BackSub: r_from_down * r1_reg (dedicated)
    DW_fp_mult_DG #(
        .sig_width(23),
        .exp_width(8),
        .ieee_compliance(1)
    ...
    // BackSub: r2' = back_sum_in - (r_from_down * r1_reg) (dedicated)
    DW_fp_sub #(
        .sig_width(23),
        .exp_width(8),
        .ieee_compliance(1)
    ...
    // Keep legacy signal names for waveform compatibility
    // sr1: select between QR/back dedicated multipliers
    assign sr1 = mode ? sr1_back : sr1_qr;

    wire [31:0] r2_prime = mode ? r2_prime_back : r2_prime_qr;
